...
The implemented WIM app and driver paths are `amd64` oriented. AzCopy and curl download only amd64 payloads, 7-Zip copies only its x64 payload, OSDeploy driver selection reads `boot-assets\winpedrivers-amd64`, and driver log entries record `amd64`.
...
| `ProgramData`  | Locates OSDeploy caches for PowerShell packages and WinPE apps, plus the unified Boot-Assets driver library.                              |
...
2. Enumerate unapplied immediate subdirectories under OSDeploy Core `boot-assets\winpedrivers-amd64`.
...
The WIM OSDeploy Core driver picker appears only when unapplied Boot-Assets drivers exist. Canceling it skips those drivers and continues.
