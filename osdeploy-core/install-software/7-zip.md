...
7-Zip provides archive support on the workstation and in OSDeploy boot images. `Install-OSDeploySoftware` installs the `7zip.7zip` WinGet package and prepares the files used to add 7-Zip to WinPE.
...
2. Installs 7-Zip on the workstation when `C:\Program Files\7-Zip\7z.exe` is not present.
...
Prepare both architecture-specific installers and the WinPE app cache without installing 7-Zip on the workstation:
